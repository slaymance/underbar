(function() {
  ...
  _.reject = function(collection, test) {
    ...
    var rejectArray = collection.slice(0);
    var trueArray = _.filter(rejectArray, test);

    _.each(trueArray, function(item) {
      rejectArray.splice(_.indexOf(rejectArray, item), 1);
    });
  ...
  _.every = function(collection, iterator) {
    ...
    var trueArray = collection;
    if (iterator) {
      trueArray = _.map(collection, iterator);
    }

    return _.reduce(trueArray, function(trueTest, item) {
      if (!item) {
        return false;
      }
      return trueTest;
  ...
  _.invoke = function(collection, functionOrKey, args) {
    return _.map(collection, function(parameter) {
      var func;
      if (typeof functionOrKey === 'function') {
        func = functionOrKey;
      } else {
        func = parameter[functionOrKey];
      }
      return func.apply(parameter, args);
    });
  };
  ...
  _.zip = function() {
    var longest = _.reduce(arguments, function(length, arg) {
  ...
  _.intersection = function() {
    ...
    var sharedArray = [];
    _.each(argArray[0], function(element) {
      var isShared = true;
      for (var i = 1; i < argArray.length; i++) {
        if (!_.contains(argArray[i], element)) {
          isShared = false;
          break;
        }
      }
